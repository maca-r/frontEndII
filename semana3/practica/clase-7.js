...
function mostrarDatosEnPerfil(listaAlbumes) {
    // desarrollar la función 👇
    let cantAlbumes = document.getElementById("cant-albums")
    let cantFavoritos = document.getElementById("cant-favoritos")

    

    let contadorAlbum = 0
    let contadorFavorito = 0
    for (let album of listaAlbumes) {
        contadorAlbum ++
        if (album.like){
            contadorFavorito += 1
        }
    ...
    if (contadorAlbum > 1){
        cantAlbumes.innerText = `${contadorAlbum} álbumes`
    } else
        cantAlbumes.innerText = `${contadorAlbum} álbum`

    if (contadorFavorito > 1){
        cantFavoritos.innerText = `${contadorFavorito} favoritos`
    } else
        cantFavoritos.innerText = `${contadorFavorito} favorito`
