...
for (let item of listadoNoticias) {
    const articulo = document.createElement('article'); //se crea un articulo por cada elementro de listadoNoticias

    if (item.titulo) { //si tiene un titulo, se crea un h2 en mayusc y se agrega al articulo
        const titulo = document.createElement('h2');
        titulo.innerText = item.titulo.toUpperCase();
        articulo.appendChild(titulo);
    }

    const imagen = document.createElement('img'); //se crea la imagen y se agrega al articulo 
    imagen.setAttribute('src', item.foto);
    articulo.appendChild(imagen);

    const parrafo = document.createElement('p'); //se crea un parrafo y se agrega al articulo
    parrafo.innerText = item.epigrafe;
    articulo.appendChild(parrafo);

    main.appendChild(articulo); //se agrega el artiuclo al main
} */
